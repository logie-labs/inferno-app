...
function Button({
  ...
  // because dropping native semantics silently breaks form submission and
  // keyboard activation. A `render` of <a> or <div> is a deliberate choice
  // though - a link styled as a button is still a link - so the flag is
  // answered from what is actually being rendered rather than left for every
  // call site to remember. A render *function* is opaque here, so it keeps the
  // safe default and can still pass `nativeButton` explicitly.
  const isNative =
